...
class GameSummaryServiceTest {
  ...
  @Mock private BuyInRepository buyInRepository;

  @Mock private StackRepository stackRepository;

  @InjectMocks private GameSummaryService summaryService;
  ...
  void shouldCalculatePlayerResultsCorrectly() {
    ...
    when(buyInRepository.findByPlayer(alice))
        .thenReturn(
            List.of(
                BuyIn.builder().amount(BigDecimal.valueOf(20)).build(),
                BuyIn.builder().amount(BigDecimal.valueOf(10)).build()));
    when(buyInRepository.findByPlayer(bob))
        .thenReturn(List.of(BuyIn.builder().amount(BigDecimal.valueOf(30)).build()));

    when(stackRepository.findByPlayer(alice))
        .thenReturn(Optional.of(Stack.builder().finalAmount(BigDecimal.valueOf(50)).build()));
    when(stackRepository.findByPlayer(bob))
        .thenReturn(Optional.of(Stack.builder().finalAmount(BigDecimal.valueOf(10)).build()));

    List<PlayerResultDTO> results = summaryService.getPlayerResults(game);

    assertThat(results).hasSize(2);
    PlayerResultDTO aliceResult =
        results.stream()
            .filter(r -> r.getPlayerId().equals(alice.getId()))
            .findFirst()
            .orElseThrow();
    PlayerResultDTO bobResult =
        results.stream().filter(r -> r.getPlayerId().equals(bob.getId())).findFirst().orElseThrow();
  ...
  void shouldGenerateCorrectSettlements() {
    ...
    List<SettlementDTO> settlements =
        summaryService.calculateSettlements(List.of(aliceResult, bobResult));
